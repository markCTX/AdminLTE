...
function quietfilter(ta,data)
{
    ...
    for(var i = 0;i<lines.length;i++)
    {
        ...
        {
            var shortstring = lines[i].replace("::: /etc/xfilter/","");
            // Remove "(x results)"
...
function eventsource() {
    ...
    if (typeof EventSource !== "function") {
        httpGet(ta,quiet,"/admin/scripts/x-filter/php/queryads.php?domain="+domain.toLowerCase()+exact+"&IE");
        return;
    }

    var host = window.location.host;
    var source = new EventSource("/admin/scripts/x-filter/php/queryads.php?domain="+domain.toLowerCase()+"&"+exact);
