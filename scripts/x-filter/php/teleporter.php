	...
	if($_FILES["zip_file"]["name"] && $_POST["action"] == "in")
	{
		...
		foreach($archive as $file)
		{
			if(isset($_POST["blacklist"]) && $file->getFilename() === "blacklist.txt")
			{
				...
				exec("sudo xfilter -b -nr --nuke");
				exec("sudo xfilter -b -q -nr ".implode(" ", $blacklist));
			...
			if(isset($_POST["whitelist"]) && $file->getFilename() === "whitelist.txt")
			{
				...
				exec("sudo xfilter -w -nr --nuke");
				exec("sudo xfilter -w -q -nr ".implode(" ", $whitelist));
				$importedsomething = true;
			}
			...
			if(isset($_POST["regexlist"]) && $file->getFilename() === "wildcardblocking.txt")
			{
				...
				exec("sudo xfilter --wild -nr --nuke");
				exec("sudo xfilter --wild -q -nr ".implode(" ", $wildlist));
				$importedsomething = true;
			}
			...
			if(isset($_POST["auditlog"]) && $file->getFilename() === "auditlog.list")
			{
				...
				exec("sudo xfilter -a clearaudit");
				exec("sudo xfilter -a audit ".implode(" ",$auditlog));
			}

			if($importedsomething)
			{
				exec("sudo xfilter restartdns");
			}
		}
...
{
	$tarname = "x-filter-teleporter_".date("Y-m-d_h-i-s").".tar";
	$filename = $tarname.".gz";
	...
	archive_add_file("/etc/xfilter/","whitelist.txt");
	archive_add_file("/etc/xfilter/","blacklist.txt");
	archive_add_file("/etc/xfilter/","adlists.list");
	archive_add_file("/etc/xfilter/","setupVars.conf");
	archive_add_file("/etc/xfilter/","auditlog.list");
	archive_add_file("/etc/xfilter/","regex.list");
