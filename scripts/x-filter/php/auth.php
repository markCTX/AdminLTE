...
function check_cors() {
    ...
    // https://x.filter
    // x.filter:8080
    // However, we don't use parse_url(...) if there is no colon, since it will fail for e.g. "x.filter"
...
function check_csrf($token) {
    ...
    if(!isset($_SESSION['token'])) {
        log_and_die("Session expired! Please re-login on the X-filter dashboard.");
    }
    ...
    if(!hash_equals($_SESSION['token'], $token)) {
        log_and_die("Wrong token! Please re-login on the X-filter dashboard.");
    }
}
