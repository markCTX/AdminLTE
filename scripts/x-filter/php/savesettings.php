...
function readStaticLeasesFile()
{
	...
	try
	{
		$dhcpstatic = @fopen('/etc/dnsmasq.d/04-xfilter-static-dhcp.conf', 'r');
	}
	catch(Exception $e)
	{
		echo "Warning: Failed to read /etc/dnsmasq.d/04-xfilter-static-dhcp.conf, this is not an error";
		return false;
	}
...
function readAdlists()
{
	// Reset list
	$list = [];
	$handle = @fopen("/etc/xfilter/adlists.list", "r");
	if ($handle)
	{
	...
	if(isset($_POST["field"]))
	{
		...
		switch ($_POST["field"]) {
			...
			case "DNS":
				...
				exec("sudo xfilter -a -i ".$DNSinterface." -web");

				// If there has been no error we can save the new DNS server IPs
				if(!strlen($error))
				{
					$IPs = implode (",", $DNSservers);
					$return = exec("sudo xfilter -a setdns \"".$IPs."\" ".$extra);
			...
			case "API":
				...
				if(!strlen($error))
				{
					...
					exec("sudo xfilter -a setexcludedomains ".$domainlist);
					exec("sudo xfilter -a setexcludeclients ".$clientlist);
				...
				{
					exec("sudo xfilter -a setquerylog all");
				...
				{
					exec("sudo xfilter -a setquerylog permittedonly");
					if(!isset($_POST["privacyMode"]))
					{
				...
				{
					exec("sudo xfilter -a setquerylog blockedonly");
					$success .= "Only blocked entries will be shown in Query Log";
				}
				else
				{
					exec("sudo xfilter -a setquerylog nothing");
					$success .= "No entries will be shown in Query Log";
				}
				...
				if(isset($_POST["privacyMode"]))
				{
					exec("sudo xfilter -a privacymode true");
					$success .= " (privacy mode enabled)";
				}
				else
				{
					exec("sudo xfilter -a privacymode false");
				}
			...
			case "webUI":
				if($_POST["tempunit"] == "F")
				{
					exec('sudo xfilter -a -f');
				}
				elseif($_POST["tempunit"] == "K")
				{
					exec('sudo xfilter -a -k');
				}
				else
				{
					exec('sudo xfilter -a -c');
				}
				...
				{
					exec('sudo xfilter -a -e \''.$adminemail.'\'');
				}
				if(isset($_POST["boxedlayout"]))
				{
					exec('sudo xfilter -a layout boxed');
				}
				else
				{
					exec('sudo xfilter -a layout traditional');
				}
				$success .= "The webUI settings have been updated";
				break;

			case "poweroff":
				exec("sudo xfilter -a poweroff");
				$success = "The system will poweroff in 5 seconds...";
				break;

			case "reboot":
				exec("sudo xfilter -a reboot");
				$success = "The system will reboot in 5 seconds...";
				break;

			case "restartdns":
				exec("sudo xfilter -a restartdns");
				$success = "The DNS server has been restarted";
				break;

			case "flushlogs":
				exec("sudo xfilter -f");
				$success = "The X-filter log file has been flushed";
				break;
			...
			case "DHCP":
				...
				if(isset($_POST["addstatic"]))
				{
					...
					if(!strlen($error))
					{
						exec("sudo xfilter -a addstaticdhcp ".$mac." ".$ip." ".$hostname);
				...
				if(isset($_POST["removestatic"]))
				{
					...
					if(!strlen($error))
					{
						exec("sudo xfilter -a removestaticdhcp ".$mac);
				...
				if(isset($_POST["active"]))
				{
					...
					if(!strlen($error))
					{
						exec("sudo xfilter -a enabledhcp ".$from." ".$to." ".$router." ".$leasetime." ".$domain." ".$ipv6);
						$success .= "The DHCP server has been activated ".htmlspecialchars($type);
					}
				}
				else
				{
					exec("sudo xfilter -a disabledhcp");
			...
			case "adlists":
				foreach ($adlist as $key => $value)
				{
					...
					{
						// Delete list
						exec("sudo xfilter -a adlist del ".escapeshellcmd($value[1]));
					}
					elseif(isset($_POST["adlist-enable-".$key]) && !$value[0])
					{
						// Is not enabled, but should be
						exec("sudo xfilter -a adlist enable ".escapeshellcmd($value[1]));

					}
					elseif(!isset($_POST["adlist-enable-".$key]) && $value[0])
					{
						// Is enabled, but shouldn't be
						exec("sudo xfilter -a adlist disable ".escapeshellcmd($value[1]));
					}
				}
				...
				if(strlen($_POST["newuserlists"]) > 1)
				{
					...
					foreach($domains as $domain)
					{
						exec("sudo xfilter -a adlist add ".escapeshellcmd($domain));
					}
				}
			...
			case "privacyLevel":
				...
				if($level >= 0 && $level <= 4)
				{
					...
					// Store privacy level
					exec("sudo xfilter -a privacylevel ".$level);
